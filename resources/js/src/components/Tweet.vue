<template>
  <a class="c-btn c-btn--m c-btn--tweet" :class="'p-' + project + '__tweet-btn'" :href="'http://twitter.com/share?url=' + url + '&text=' + text" target="_blank" rel="noopener noreferrer" @click="generateText">
    <svg version="1.1" xmlns="http://www.w3.org/2000/svg" class="c-btn--tweet__icon" :class="'p-' + project + '__tweet-icon'" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="128 128 256 256" xml:space="preserve">
...
export default {
  ...
  data(){
    return {
      url: encodeURIComponent(this.dest),
      text: '',
      sharePhrase: [
        'これが私のおすすめ！',
        'これ推します！',
  ...
  methods: {
    //シェアボタンを押したときに文章を生成する
    generateText(){
      const phrase = this.sharePhrase[Math.floor(Math.random() * this.sharePhrase.length)] + '%0a%0a'
      this.text = phrase + encodeURIComponent(this.title + '\n\n[ウェブサービス' +  this.appName + ']\n〜' + this.catchphrase + '〜') + '%0a'
    }
  }
}
</script>